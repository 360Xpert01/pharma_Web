...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-8 text-sm font-medium transition-all cursor-pointer disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        default:
      ...
      size: {
        ...
        sm: "h-8 rounded-8 gap-1.5 px-3 text-xs",
        lg: "h-11 rounded-8 px-6 text-base",
        xl: "h-12 rounded-8 px-8 text-lg",
        icon: "size-9",
        "icon-sm": "size-8",
        "icon-lg": "size-11",
      },
...
export interface ButtonProps
  ...
  /** Required action permission(s) - button will hide if permission is missing */
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    if (action) {
      ...
      const hasPermission = actions.some((a) => canDo(a));
