...
export default function AddDistributorTypesCard({
  ...
  const handleSubmit = async () => {
    ...
    if (!isUpdateMode) {
      // Validate form without pulse code for creation (it will be generated by backend)
      try {
    ...
    try {
      if (isUpdateMode && updateId) {
        ...
        // Create new distributor type - send name and status (pulseCode generated by backend)
        const res = await dispatch(
          createDistributorType({
            name: distributorTypeName.trim(),
            status,
