...
export const employeeRegistrationSchema = z
  ...
  .superRefine((data, ctx) => {
    // Conditional validation: Territory is required if role is "Sales Rep"
    if (data.roleName && data.roleName.toLowerCase().includes("sales rep")) {
      if (!data.territoryId || data.territoryId === "") {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Territory is required for Sales Rep role",
          path: ["territoryId"],
        });
      }
    }
